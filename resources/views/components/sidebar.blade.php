        ...
        <nav class="space-y-1.5">
            ...
            <div class="relative group">
                ...
                <a href="{{ route('admin.dashboard') }}" class="flex items-center justify-between px-3 py-2.5 rounded-lg
                    ...
                    <div class="flex items-center gap-3">
                        <svg class="w-5 h-5 {{ request()->routeIs('admin.dashboard') ? 'text-donezo-primary' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div>
                <button @click="masterOpen = !masterOpen" class="w-full flex items-center justify-between px-3 py-2.5 rounded-lg
                    {{ request()->routeIs('admin.manage-classes', 'admin.rules', 'admin.students', 'admin.teachers', 'admin.parents', 'admin.manage-schedules')
                        ? 'text-donezo-text font-bold bg-gray-50'
                        : 'text-gray-500 font-medium hover:text-donezo-text hover:bg-gray-50'
                    }} transition-colors">
                    <div class="flex items-center gap-3">
                        <svg class="w-5 h-5 {{ request()->routeIs('admin.manage-classes', 'admin.rules', 'admin.students', 'admin.teachers', 'admin.parents', 'admin.manage-schedules') ? 'text-donezo-primary' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4"></path>
                        </svg>
                        <span>Data Master</span>
                    </div>
                    <svg class="w-4 h-4 transition-transform duration-200" :class="{'rotate-180': masterOpen}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                    </svg>
                </button>

                <div x-show="masterOpen" x-collapse class="mt-1 space-y-1 pl-11 pr-2">
                    <a href="{{ route('admin.manage-classes') }}" class="block px-3 py-2 text-sm rounded-lg {{ request()->routeIs('admin.manage-classes') ? 'text-donezo-primary font-bold bg-teal-50' : 'text-gray-500 hover:text-donezo-text hover:bg-gray-50' }} transition-colors">Data Kelas</a>
                    <a href="{{ route('admin.rules') }}" class="block px-3 py-2 text-sm rounded-lg {{ request()->routeIs('admin.rules') ? 'text-donezo-primary font-bold bg-teal-50' : 'text-gray-500 hover:text-donezo-text hover:bg-gray-50' }} transition-colors">Data Peraturan</a>
                    <a href="{{ route('admin.students') }}" class="block px-3 py-2 text-sm rounded-lg {{ request()->routeIs('admin.students') ? 'text-donezo-primary font-bold bg-teal-50' : 'text-gray-500 hover:text-donezo-text hover:bg-gray-50' }} transition-colors">Data Siswa</a>
                    <a href="{{ route('admin.teachers') }}" class="block px-3 py-2 text-sm rounded-lg {{ request()->routeIs('admin.teachers') ? 'text-donezo-primary font-bold bg-teal-50' : 'text-gray-500 hover:text-donezo-text hover:bg-gray-50' }} transition-colors">Data Guru</a>
                    <a href="{{ route('admin.parents') }}" class="block px-3 py-2 text-sm rounded-lg {{ request()->routeIs('admin.parents') ? 'text-donezo-primary font-bold bg-teal-50' : 'text-gray-500 hover:text-donezo-text hover:bg-gray-50' }} transition-colors">Data Orang Tua</a>
                    <a href="{{ route('admin.manage-schedules') }}" class="block px-3 py-2 text-sm rounded-lg {{ request()->routeIs('admin.manage-schedules') ? 'text-donezo-primary font-bold bg-teal-50' : 'text-gray-500 hover:text-donezo-text hover:bg-gray-50' }} transition-colors">Jadwal Piket</a>
                </div>
            </div>
            @endif
            ...
            <div class="relative group">
                ...
                <a href="{{ $violationTargetRoute }}" class="flex items-center justify-between px-3 py-2.5 rounded-lg {{ $isViolationRoute ? 'text-donezo-text font-bold bg-gray-50' : 'text-gray-500 font-medium hover:text-donezo-text hover:bg-gray-50' }} transition-colors">
                    <div class="flex items-center gap-3">
                        <svg class="w-5 h-5 {{ $isViolationRoute ? 'text-donezo-primary' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-5M18.364 5.364a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.364-9.364z"></path>
                        </svg>
            ...
            <div class="relative group">
                @php
                    ...
                    $verifyTargetRoute = $user->hasRole('admin') ? route('admin.verify-violations') : route('teacher.verify-violations');
                @endphp
                <div class="absolute -left-4 top-1/2 -translate-y-1/2 w-1.5 h-8 bg-donezo-primary rounded-r-md {{ $isVerifyRoute ? 'block' : 'hidden' }}"></div>
                <a href="{{ $verifyTargetRoute }}" class="flex items-center justify-between px-3 py-2.5 rounded-lg {{ $isVerifyRoute ? 'text-donezo-text font-bold bg-gray-50' : 'text-gray-500 font-medium hover:text-donezo-text hover:bg-gray-50' }} transition-colors">
                    <div class="flex items-center gap-3">
                        <svg class="w-5 h-5 {{ $isVerifyRoute ? 'text-donezo-primary' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">

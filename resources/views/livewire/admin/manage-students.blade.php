<div class="p-6 lg:p-10 bg-[#FAFBFC] min-h-screen">
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500 font-medium">Kelola data siswa, perbarui kelas, dan atur akun login siswa.</p>
        </div>
            ...
            <div class="lg:col-span-4 animate-in fade-in slide-in-from-left-4 duration-300">
                <div class="bg-white border border-gray-100 rounded-[2rem] p-8 shadow-sm sticky top-8">
                    <h3 class="text-lg font-bold text-slate-800 mb-6">
                        {{ $isEdit ? 'Update Data Siswa' : 'Input Siswa Baru' }}</h3>
                    ...
                    <form wire:submit.prevent="save" class="space-y-5">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 ml-1">Email Sekolah (Untuk Login)</label>
                        ...
                        <div class="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                            <p class="text-xs text-gray-500 leading-relaxed">
                                💡 <strong>Info:</strong> Password login siswa akan otomatis diset dari <strong>NIS</strong> mereka.
                            </p>
                        </div>
            ...
            <div class="bg-white border border-gray-100 rounded-[2rem] shadow-sm overflow-hidden">
                <div class="p-6 border-b border-gray-50 flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div class="relative w-full sm:w-64">
                        ...
                        <input wire:model.live="search" type="text" placeholder="Cari nama atau NIS..."
                    ...
                    <table class="w-full text-left">
                        <thead>
                            <tr class="text-[11px] font-bold text-gray-400 uppercase tracking-[0.2em] bg-gray-50/50">
                                ...
                                <th class="px-8 py-5">Email / NIS</th>
                                <th class="px-8 py-5">Kelas / Wali</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                                    ...
                                    <td class="px-8 py-5">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="flex flex-col">
                                                <span
                                                    class="font-bold text-slate-700 leading-tight group-hover:text-teal-600 transition-colors">{{ $student->name }}</span>
                                                <span class="text-[10px] text-gray-400 mt-1 uppercase tracking-wider font-semibold">Total Poin: {{ $student->total_points ?? 0 }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-8 py-5 text-sm text-gray-600 font-medium">
                                        <div class="flex flex-col">
                                            <span>{{ $student->user?->email ?? 'Tidak ada email' }}</span>
                                            <span class="text-[10px] text-gray-400">NIS: {{ $student->nis }}</span>
                                        </div>
                                    </td>
                                    <td class="px-8 py-5 text-sm text-gray-600 font-medium">
                                        <div class="flex flex-col gap-1">
                                            <span>Kelas: {{ $student->classRoom?->name ?? '-' }}</span>
                                            <span>Ortu: {{ $student->parent?->name ?? '-' }}</span>
                                        </div>
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-8 py-20 text-center text-gray-400 italic">Belum ada data siswa.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>

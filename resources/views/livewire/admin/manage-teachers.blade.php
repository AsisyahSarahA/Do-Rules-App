<div class="p-6 lg:p-10 bg-[#FAFBFC] min-h-screen">
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500 font-medium">Kelola data tenaga pengajar, perbarui jabatan, dan kelola akun.
            </p>
        </div>
            ...
            <div class="lg:col-span-4 animate-in fade-in slide-in-from-left-4 duration-300">
                <div class="bg-white border border-gray-100 rounded-[2rem] p-8 shadow-sm sticky top-8">
                    <h3 class="text-lg font-bold text-slate-800 mb-6">
                        {{ $isEdit ? 'Update Data Akun' : 'Input Guru Baru' }}</h3>

                    <form wire:submit.prevent="save" class="space-y-5">
                        <div>
                            <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 ml-1">NIP
                                Guru</label>
                            <input wire:model="nip" type="text" placeholder="Masukkan NIP"
                        ...
                        <div>
                            <label
                                ...
                                Lengkap</label>
                            <input wire:model="name" type="text" placeholder="Misal: Dr. Hj. Asisyah Sarah"
                                class="w-full px-5 py-3.5 bg-gray-50 border-none focus:ring-2 focus:ring-teal-900/10 rounded-2xl text-sm transition-all">
                            @error('name')
                                <span class="text-xs text-red-500 mt-1 block ml-1">{{ $message }}</span>
                            @enderror
                        ...
                        <div>
                            <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 ml-1">No.
                                HP (WhatsApp)</label>
                        ...
                        @if (!$isEdit)
                            <div class="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                                <p class="text-xs text-gray-500 leading-relaxed">
                                    💡 <strong>Info:</strong> Menambahkan guru baru akan otomatis membuatkan akun login
                                    dengan password default menggunakan <strong>NIP Guru</strong> yang Anda masukkan di
                                    atas.
                                </p>
                            </div>
                        @endif
                        ...
                        <div class="pt-2">
                            <button type="submit" wire:loading.attr="disabled"
                                ...
                                <span wire:loading.remove
                                    wire:target="save">{{ $isEdit ? 'Simpan Perubahan' : 'Daftarkan Akun & Guru' }}</span>
            ...
            <div class="bg-white border border-gray-100 rounded-[2rem] shadow-sm overflow-hidden">
                <div class="p-6 border-b border-gray-50 flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div class="relative w-full sm:w-64">
                        ...
                        <input wire:model.live="search" type="text" placeholder="Cari nama atau NIP..."
                    ...
                    <table class="w-full text-left">
                        <thead>
                            <tr class="text-[11px] font-bold text-gray-400 uppercase tracking-[0.2em] bg-gray-50/50">
                                ...
                                <th class="px-8 py-5">Email / NIP</th>
                            ...
                            @forelse($teachers as $teacher)
                                <tr class="group hover:bg-gray-50/50 transition-all">
                                    <td class="px-8 py-5">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="flex flex-col">
                                                <span
                                                    class="font-bold text-slate-700 leading-tight group-hover:text-teal-600 transition-colors">{{ $teacher->name }}</span>
                                                <span
                                                    class="text-[10px] text-gray-400 mt-1 uppercase tracking-wider font-semibold">Telp:
                                                    {{ $teacher->phone ?? '-' }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-8 py-5 text-sm text-gray-600 font-medium">
                                        <div class="flex flex-col">
                                            <span>{{ $teacher->email ?? 'Tidak ada email' }}</span>
                                            <span class="text-[10px] text-gray-400">NIP: {{ $teacher->nip }}</span>
                                        </div>
                                    </td>
                                    <td class="px-8 py-5">
                                        <div class="relative max-w-[160px]">
                                            <select wire:change="setRole({{ $teacher->id }}, $event.target.value)"
                                                ...
                                                <option value="guru"
                                                    {{ optional($teacher->user)->role == 'guru' ? 'selected' : '' }}>
                                                    👨‍🏫 Guru</option>
                                                <option value="wali_kelas"
                                                    {{ optional($teacher->user)->role == 'wali_kelas' ? 'selected' : '' }}>
                                                    📋 Wali Kelas</option>
                                                <option value="piket"
                                                    {{ optional($teacher->user)->role == 'piket' ? 'selected' : '' }}>
                                                    🛡️ Piket</option>
                                            </select>
                                            <div
